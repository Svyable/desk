...
assert "function scheduleOptionalEnhancements()" in loader
assert "for (const [url, label] of optionalEnhancements)" in loader
assert "import(url).catch((error) =>" in loader
assert "window.requestIdleCallback(loadOptionalEnhancements, { timeout: 750 });" in loader
assert "window.setTimeout(loadOptionalEnhancements, 0);" in loader
assert "await import(canonicalAppUrl);" in loader
assert "scheduleOptionalEnhancements();" in loader
...
startup = loader[loader.rindex("try {") :]
assert startup.index("await import(canonicalAppUrl);") < startup.index("scheduleOptionalEnhancements();")
...
assert "loadOptionalEnhancements();" not in startup, "optional network work must stay off the immediate app path"
assert "showRecovery(error);" in startup, "canonical app failure must retain accessible recovery"

print("Desk Reader blocks only on canonical app startup and defers optional polish to browser idle time")
