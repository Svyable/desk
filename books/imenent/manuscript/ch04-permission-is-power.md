...
That difference matters most when the system is under stress. If a human can revoke the visible account in thirty seconds but needs two hours to discover the credentials, queued work, and commitments that account created, then the meaningful control time is not thirty seconds. It is two hours.

Re-entry is therefore partly a latency property of the permission system. The relevant question is how quickly accountable humans can contract the *effective* action envelope, not how quickly an interface can change a status field.

That test also changes what counts as observability. A dashboard generated by the same agent layer may be useful, but it is a weak sole source of truth during a revocation event. The institution needs a sufficiently independent record of identities, grants, descendants, and durable actions to know what still has to be unwound.
