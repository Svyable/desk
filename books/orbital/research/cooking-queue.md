# ORBITAL — COOKING QUEUE

## Immediate manuscript
- [x] Book thesis and architecture
- [x] Prologue first draft
- [x] Chapter 1 first draft
- [x] Chapter 2 first draft
- [x] Chapter 3 first draft
- [x] Chapter 4 first draft
- [x] Interlude: orbital geography
- [ ] Chapter 5 — The Factory Learns to Fly
- [ ] Chapter 6 — constellation precursors and why earlier LEO telecom failed
- [ ] Chapter 7 — Starlink deployment flywheel

## Research packets to build before deep drafting
- [ ] COTS primary documents / Space Act Agreements / NASA oral histories
- [ ] CRS award chronology and dollars
- [ ] Commercial Crew contract chronology and exact fixed-price / certification structure
- [ ] Falcon 9 reuse milestones and cadence table, 2010–2026
- [ ] Booster turnaround / flight-count primary sources
- [ ] Starlink constellation population by year and generation
- [ ] Starlink revenue/customer estimates — source quality matrix
- [ ] FCC authorizations and modification orders
- [ ] ESA debris reports 2024–2026
- [ ] Commercial remote sensing market / Ukraine chronology
- [ ] Commercial LEO destination contracts and ISS transition plan
- [ ] China LEO constellation / commercial-space policy research

## Editorial passes
- [ ] Reduce repeated use of “architecture” and “system” once full draft exists
- [ ] Build character scenes around procurement officials and engineers
- [ ] Add moments of physical specificity inside factories/control rooms
- [ ] Separate “SpaceX dominance” from “commercial industry dominance” with quantitative measures
- [ ] Stress-test title against Samantha Harvey's *Orbital*
- [ ] Formal endnotes and bibliography
