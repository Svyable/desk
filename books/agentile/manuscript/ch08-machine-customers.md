...
The demand side is less settled. In a Harris Poll survey commissioned by Visa and fielded in May 2026, only 23 percent of U.S. respondents said they trusted generative AI to handle payment transactions on their behalf. The same survey found much greater trust when a familiar payment network stood behind the transaction. That is interested industry research, not a neutral adoption forecast, but the contrast matters. A protocol can make delegated payment technically possible long before people are willing to delegate the authority.
...
Money makes the boundary harder. A conventional card transaction establishes that a credential was presented under certain conditions. An agentic transaction adds another question: was this software authorized to make *this* purchase for *this* principal? Google’s Agent Payments Protocol makes that question almost physical. For a delegated purchase, a user can sign an intent mandate describing constraints such as price and timing; later transaction records can bind a cart and payment to that authorization.

That solves an important problem, but not the whole one. A cryptographic signature can prove that a particular instruction or cart was approved. It cannot, by itself, prove that the path to that approval was honest. In August 2026, a security analysis of the protocol’s then-current design mapped attacks in which manipulated messages or tool outputs changed the context *before* authorization while leaving the later signed mandates internally valid. The uncomfortable possibility is a perfectly authenticated mistake.

This separates two questions that are easy to fuse: *Was the agent allowed to spend?* and *Did the agent faithfully represent what the principal meant?* Payment infrastructure can get much better at the first while the second remains a problem of retrieval, software supply chains, model behavior, merchant incentives, and interface design. Intelligence is insufficient. So is permission by itself.
