...
- [ ] Run `python3 scripts/check-book-length.py who-taught-america` on one frozen Desk source and require `healthy: true`, every numbered chapter at or above 3,000 words, and at least 65,000 chapter-only words.
...
- [ ] Perform the final manuscript-to-source reconciliation on the frozen candidate and correct any claim that outruns its packet.
...
- [ ] After the exact length report, deepen any chapter below the floor with researched cases, counter-evidence, institutional mechanics, or downstream consequences—not restatement or padding.

## Gate 4 — Desk catalog and discovery

- [x] Add `who-taught-america` to the canonical `catalog.json` manifest from current `main`.
- [ ] Regenerate or synchronize the root README catalog/dashboard from the same source.
- [x] Add the book to `.github/ISSUE_TEMPLATE/chapter-feedback.yml`.
- [x] Add the canonical book route to `llms.txt` and `sitemap.xml`.
- [ ] Run `python3 scripts/check-desk.py` and require a clean result.
...
- [ ] Run `python3 scripts/check-desk.py` on the same frozen source and record the result.
...
Do not publish from a moving branch. Publish only from the named frozen Desk candidate after the exact book-length and Desk-integrity checks pass. No discretionary human-review blocker is added: once the objective gates recorded here are green, the first-edition release may proceed under the repository’s normal merge and release protections.
