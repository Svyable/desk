# Intelligence State

*Power, Prosperity, and the AI Future of the Americas*

<!-- bookself-reader-links:start -->
**Reader links:** [Working edition · Desk Reader](https://svyable.github.io/desk/reader/#/b/intelligence-state/) · [Published edition · Shelf Reader](https://svyable.github.io/shelf/reader/#/b/intelligence-state/)
<!-- bookself-reader-links:end -->

| | |
|---|---|
| **Author** | Sven Hardy Benson |
| **Status** | Complete draft · Published on Shelf |
| **Chapters** | 17 of 17 + prologue + conclusion |
| **Publication** | First edition · September 2026 |
| **Public Reader** | [Read the published Shelf edition →](https://svyable.github.io/shelf/reader/#/b/intelligence-state/) |
| **Method** | Narrative nonfiction + institutional analysis + current primary-source research |

Artificial intelligence is becoming a form of state capacity.

*Intelligence State* follows that transformation across the Americas and asks a harder question than who builds the best model: **who can make intelligence abundant without making sovereignty scarce?** The book treats sovereignty not as autarky but as meaningful choice under pressure across five currencies—compute, energy, data, talent, and trust.

This Desk copy remains the working source for future revisions. The first public edition was intentionally snapshotted to [Sven Hardy Benson’s Shelf](https://github.com/Svyable/shelf/tree/main/books/intelligence-state) in September 2026; later Desk edits do not silently rewrite that edition.

## Contents

- [x] [Front Matter](manuscript/front-matter.md)
- [x] [Prologue — The Border Crossing](manuscript/prologue-the-border-crossing.md)

### Part I — The New Map of Power

- [x] [Ch 1 — The Country That Thinks](manuscript/ch01-the-country-that-thinks.md)
- [x] [Ch 2 — The Five Currencies of Sovereignty](manuscript/ch02-the-five-currencies-of-sovereignty.md)
- [x] [Ch 3 — The American Engine](manuscript/ch03-the-american-engine.md)
- [x] [Ch 4 — Canada and the Sovereignty Premium](manuscript/ch04-canada-and-the-sovereignty-premium.md)

### Part II — The Hemisphere Becomes a Machine

- [x] [Ch 5 — Mexico: The Factory Learns to Think](manuscript/ch05-mexico-the-factory-learns-to-think.md)
- [x] [Ch 6 — The Caribbean: Small States, Big Intelligence](manuscript/ch06-the-caribbean-small-states-big-intelligence.md)
- [x] [Ch 7 — Chile, Uruguay, and the Competent State](manuscript/ch07-chile-uruguay-and-the-competent-state.md)
- [x] [Ch 8 — Brazil: The Green Compute Republic](manuscript/ch08-brazil-the-green-compute-republic.md)
- [x] [Ch 9 — The Amazon Has a Data Policy](manuscript/ch09-the-amazon-has-a-data-policy.md)
- [x] [Ch 10 — Andes: The Mineral Inside the Machine](manuscript/ch10-andes-the-mineral-inside-the-machine.md)

### Part III — Who Gets the Productivity?

- [x] [Ch 11 — The Eighty Percent Problem](manuscript/ch11-the-eighty-percent-problem.md)
- [x] [Ch 12 — The New Middle Class Is Augmented](manuscript/ch12-the-new-middle-class-is-augmented.md)
- [x] [Ch 13 — Government Gets an Agent](manuscript/ch13-government-gets-an-agent.md)
- [x] [Ch 14 — The Language Border](manuscript/ch14-the-language-border.md)

### Part IV — The Politics of Machine Power

- [x] [Ch 15 — The Grid Is the Computer](manuscript/ch15-the-grid-is-the-computer.md)
- [x] [Ch 16 — Democracy at Machine Speed](manuscript/ch16-democracy-at-machine-speed.md)
- [x] [Ch 17 — The Hemisphere Bargain](manuscript/ch17-the-hemisphere-bargain.md)
- [x] [Conclusion — The State After Software](manuscript/conclusion-the-state-after-software.md)
- [x] [Back Matter](manuscript/back-matter.md)

## Core argument

The first AI race was a race among laboratories. The second is a race among organizations redesigning work. The third is a race among political economies.

The book's recurring concepts include **meaningful choice under pressure**, the **sovereignty premium**, **minimum viable sovereignty**, the **competence premium**, **maximum learning per installation**, **resource curse 2.0**, **diffusion infrastructure**, **machine speed with human contestability**, **high-quality friction**, and **capability residue**.

## Research

- [Book brief and reporting architecture](research/book-brief.md)
- [Source notes](research/source-notes.md)
- [Publication research addendum](research/publication-addendum.md)

Time-sensitive investment totals, electricity projections, employment-exposure estimates, program status, and technical-standard versions are explicitly flagged for re-verification in later editions.
