...
match(boundary, /void applyAuthoringBoundary\(remoteInspection, initialRemote\)/);
...
match(boundary, /async function loadRemoteInspectionRole\(repository = remoteRepository\(\)\)/);
...
match(boundary, /function showAuthoringTools\(\)/);
match(boundary, /bookself:desk-workspace-loaded/);
match(boundary, /bookself:desk-workspace-failed/);
match(boundary, /applyAuthoringBoundary\(Boolean\(committedRemote\), committedRemote\)/);
match(boundary, /if \(remote \|\| role === 'shelf'\) hideAuthoringTools\(\);\s*else showAuthoringTools\(\);/s);
