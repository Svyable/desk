const STYLE_HREF = new URL('../css/desk-app-shell-polish.css?v=bookself-20260906', import.meta.url).href;
...
export function watchAppShellControls(document = globalThis.document) {
  ...
  const roots = [document.getElementById?.('readerChrome'), document.getElementById?.('pageNav')].filter(Boolean);
...
export function installAppShellStyles(document = globalThis.document) {
  ...
  const existing = [...document.querySelectorAll('link[rel="stylesheet"][href]')].find((link) => link.href === STYLE_HREF);
  if (existing) {
...
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  installAppShellStyles(document).catch((error) => {
    console.warn('Desk Reader app-shell polish could not be loaded', error);
  });
}
